...
router.post('/time_in',upload.single('image'),auth.authMiddleware, async (req, res) => {
    try {
        ...
        const timeIn = await AttendanceService.recordAttendance(csrId, username, role, attendanceStatus,description,filename,filePath,ipAddress)
        console.log({timeIn});
        if (timeIn) {
...
router.post('/time_out',upload.single('image'),auth.authMiddleware,async (req, res) => {
    try {
        ...
        req.session.status = attendanceStatus
        const timeOut = await AttendanceService.recordAttendance(csrId, username, role, attendanceStatus,description,filename,filePath,ipAddress)
        if (timeOut) {
            return res.status(200).redirect('/csr/attendance/form')
        }
